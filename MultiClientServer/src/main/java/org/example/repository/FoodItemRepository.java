...
/*
    ...
    public int addFoodItem(FoodItem foodItem) throws SQLException {
        String sql = "INSERT INTO fooditems (MealTypeId, Name, Price, IsAvailable, IsDelete, FoodType, SpiceLevel, Originality, SweetTooth) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            stmt.setInt(1, foodItem.getMealTypeId());
            stmt.setString(2, foodItem.getName());
            stmt.setBigDecimal(3, foodItem.getPrice());
    ...
    public void updateFoodItem(FoodItem foodItem) throws SQLException {
        String sql = "UPDATE fooditems SET MealTypeId = ?, Name = ?, Price = ?, IsAvailable = ?, IsDelete = ?, FoodType = ?, SpiceLevel = ?, Originality = ?, SweetTooth = ? WHERE Id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, foodItem.getMealTypeId());
            stmt.setString(2, foodItem.getName());
            stmt.setBigDecimal(3, foodItem.getPrice());
