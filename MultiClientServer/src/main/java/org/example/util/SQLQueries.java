...
/*public class SQLQueries {
    public static final String CHECK_VOTED_ITEM = "SELECT COUNT(*) FROM VotedItems WHERE FoodItemId = ? AND DATE(Date) = CURDATE()";
    public static final String INSERT_VOTED_ITEM = "INSERT INTO VotedItems (FoodItemId, Vote, Date, IsPrepared, IsDelete) VALUES (?, 0, NOW(), false, false)";
    public static final String UPDATE_VOTE = "UPDATE VotedItems SET Vote = Vote + 1 WHERE FoodItemId = ? AND DATE(Date) = CURDATE()";
    public static final String MARK_AS_PREPARED = "UPDATE VotedItems SET IsPrepared = true WHERE FoodItemId = ?";
    public static final String GET_MEAL_TYPE_ID = "SELECT Id FROM mealtypes WHERE Type = ? AND IsDelete = 0";
    public static final String GET_FOOD_ITEMS_FOR_VOTE = "SELECT fi.* FROM fooditems fi JOIN voteditems vi ON fi.Id = vi.FoodItemId WHERE DATE(vi.Date) = CURDATE() AND fi.MealTypeId = ?";
    public static final String GET_ROLL_OUT_ITEMS = "SELECT fi.Id, fi.MealTypeId, fi.Name, fi.Price, fi.IsAvailable, fi.IsDelete, vi.Vote FROM fooditems fi JOIN voteditems vi ON fi.Id = vi.FoodItemId WHERE DATE(vi.Date) = CURDATE() AND fi.MealTypeId = ?";
    public static final String GET_PREPARED_FOOD_ITEM_IDS = "SELECT FoodItemId FROM voteditems WHERE IsPrepared = 1 AND DATE(Date) = ?";
    public static final String GET_FOOD_ITEMS_BY_IDS_TEMPLATE = "SELECT * FROM fooditems WHERE Id IN (%s) AND IsDelete = 0";

    // Queries for AuthenticationRepository
    public static final String FIND_USER_BY_EMAIL_AND_NAME = "SELECT * FROM Users WHERE Email = ? AND Name = ? AND IsDelete = FALSE";
    public static final String FIND_ROLE_BY_ID = "SELECT * FROM Roles WHERE Id = ? AND IsDelete = FALSE";

    // Queries for FeedbackRepository
    public static final String GET_FOOD_ITEM_RATINGS = "SELECT FoodItemId, AVG(Rating) AS average_rating, GROUP_CONCAT(Comment SEPARATOR ', ') AS comments FROM feedbacks WHERE Date = CURDATE() AND FoodItemId = ? GROUP BY FoodItemId";
    public static final String UPDATE_ITEM_AUDIT = "UPDATE fooditemaudit SET Rating = ?, Sentiment = ? WHERE FoodItemId = ?";
    public static final String ADD_FEEDBACK = "INSERT INTO Feedbacks (FoodItemId, UserId, Rating, Comment, Date, IsDelete) VALUES (?, ?, ?, ?, CURDATE(), ?)";


    public static final String INSERT_USER_PROFILE = "INSERT INTO UserProfile (UserId, FoodType, SpiceLevel, Originality, SweetTooth) VALUES (?, ?, ?, ?, ?)";
    public static final String SELECT_USER_PROFILE = "SELECT FoodType, SpiceLevel, Originality, SweetTooth FROM UserProfile WHERE UserId = ?";


    public static final String SELECT_FOODITEM_NAME_MEALTYPEID = "SELECT Name, MealTypeId FROM fooditems WHERE Id = ? AND IsDelete = 0";
    public static final String SELECT_NOTIFICATION_TYPE_ID = "SELECT Id FROM NotificationTypes WHERE Type = ? AND IsDelete = 0";
    public static final String SELECT_MEAL_TYPE = "SELECT Type FROM MealTypes WHERE Id = ? AND IsDelete = 0";
    ...
    public static final String INSERT_NOTIFICATION = "INSERT INTO Notifications (NotificationTypeId, Message, IsDelete, Date) VALUES (?, ?, 0, NOW())";


    // FoodItemAuditRepository SQL query
    public static final String INSERT_FOOD_AUDIT = "INSERT INTO fooditemaudit (FoodItemId, Rating, Vote, Sentiment, Prepared) VALUES (?, ?, ?, ?, ?)";



    public static final String DELETE_FOOD_ITEM = "DELETE FROM fooditems WHERE Id = ?";
    public static final String INSERT_FOOD_ITEM = "INSERT INTO fooditems (MealTypeId, Name, Price, IsAvailable, IsDelete, FoodType, SpiceLevel, Originality, SweetTooth) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    public static final String UPDATE_FOOD_ITEM = "UPDATE fooditems SET MealTypeId = ?, Name = ?, Price = ?, IsAvailable = ?, IsDelete = ?, FoodType = ?, SpiceLevel = ?, Originality = ?, SweetTooth = ? WHERE Id = ?";
    public static final String SELECT_ALL_FOOD_ITEMS = "SELECT * FROM FoodItems";
    public static final String SELECT_RECOMMENDED_FOOD_ITEMS = "SELECT fi.* " +
            ...
            "LIMIT ?";
    public static final String SELECT_FOOD_ITEMS_BY_IDS = "SELECT * FROM foodItems WHERE Id IN (%s)";
    public static final String SELECT_DISCARDED_FOOD_ITEMS = "SELECT foodItemId FROM discarditem WHERE date >= ?";
    public static final String INSERT_DISCARDED_ITEMS = "INSERT INTO discarditem (foodItemId, date) VALUES (?, ?)";
    public static final String DELETE_FOOD_ITEMS = "DELETE FROM foodItems WHERE Id = ?";
